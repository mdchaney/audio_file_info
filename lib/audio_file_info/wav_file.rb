    ...
    def initialize(original_filename)
      ...
      File.open(original_filename, "rb") do |file|
        ...
        if (filelen + 8 - actual_file_size).abs > 1
          ...
          @error_msg = "File size error for WAV, header says #{filelen + 8}, "\
            "actual is #{actual_file_size}"
          return nil
        end
        ...
        total_chunk_length = 12 + @chunks.size * 8 +
          @chunks.map(&:length).inject(0) { |s, n| s + n } +
          @chunks.select { |ci| ci.length.odd? }.size
        if total_chunk_length == actual_file_size ||
            (total_chunk_length - 1 == actual_file_size &&
              @chunks.last.length.odd?)
          @duration =
            data_chunk.number_of_sample_frames.to_f / fmt_chunk.sample_rate.to_f
          @valid = @content_valid = true
        else
          @valid = @content_valid = false
          @error_msg = "Total chunk length #{total_chunk_length} is invalid: "\
            "expected #{actual_file_size}"
        end
      end
    end
  end
